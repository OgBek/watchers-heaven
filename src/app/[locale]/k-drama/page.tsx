...
export default function KDramaPage() {
  ...
  const loadKDramas = useCallback(async () => {
    setLoading(true);
    try {
      if (searchQuery.trim()) {
        ...
        if (data.results) {
          // Filter shows client-side to ensure Korean Drama flavor
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                Explore trending Korean dramas with filters
              </p>
            </div>
            <button
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search K-drama by title..."
        ...
        {(selectedGenres.length > 0 || selectedYear) && !showFilters && (
          <div className="flex flex-wrap items-center gap-2">
            {selectedGenres.map(gId => {
              const genre = DRAMA_GENRES.find(g => g.id === gId);
              return (
